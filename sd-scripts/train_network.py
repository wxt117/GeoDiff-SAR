...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        if args.dim_from_weights:
            network, _ = network_module.create_network_from_weights(args.network_multiplier, args.network_weights, vae,
                                                                    text_encoder, unet, **net_kwargs)
        else:
        ...
        try:
            if support_multiple_lrs:
                results = network.prepare_optimizer_params_with_multiple_te_lrs(text_encoder_lr, args.unet_lr,
                                                                                args.learning_rate)
            else:
        ...
        # === 将三模态融合网络参数加入优化器（可选） ===
        try:
            sb = importlib.import_module('library.strategy_base')
            enc_strategy = sb.TextEncodingStrategy.get_strategy()
            if getattr(args, 'enable_3d_fusion', False) and getattr(args, 'train_fusion', False) and enc_strategy is not None:
                if hasattr(enc_strategy, 'three_modal_fusion') and enc_strategy.three_modal_fusion is not None:
                    ...
                    if len(fusion_params) > 0:
                        fusion_lr = args.fusion_lr if getattr(args, 'fusion_lr', None) else args.learning_rate
                        trainable_params.append({"params": fusion_params, "lr": fusion_lr})
                        accelerator.print(f"Add fusion parameters to optimizer: {len(fusion_params)} params, lr={fusion_lr}")
        ...
        def save_model(ckpt_name, unwrapped_nw, steps, epoch_no, force_sync_upload=False):
            ...
            try:
                ...
                if hasattr(enc_strategy, 'three_modal_fusion') and enc_strategy.three_modal_fusion is not None:
                    ...
                    fusion_weights_path = ckpt_file.replace('.safetensors', '_fusion_weights.safetensors')
                    save_file(fusion_sd, fusion_weights_path)
                    logger.info(f"Saved fusion network weights -> {fusion_weights_path}")
            ...
            for step, batch in enumerate(skipped_dataloader or train_dataloader):
                ...
                with accelerator.accumulate(training_model):
                    ...
                    if "latents" in batch and batch["latents"] is not None:
                        latents = batch["latents"].to(accelerator.device, non_blocking=True).to(dtype=weight_dtype,
                                                                                                non_blocking=True)
                    else:
                        with torch.no_grad():
                            # latentに変換
                            latents = self.encode_images_to_latents(args, accelerator, vae,
                                                                    batch["images"].to(vae_dtype, non_blocking=True))
                    ...
                    if len(text_encoder_conds) == 0 or text_encoder_conds[0] is None or train_text_encoder:
                        ...
                        with torch.set_grad_enabled(train_text_encoder), accelerator.autocast():
                            ...
                            # 为3D融合准备图像路径
                            image_paths = batch.get("absolute_paths", None)
                            ...
                            if args.weighted_captions:
                                ...
                                encoded_text_encoder_conds = text_encoding_strategy.encode_tokens_with_weights(
                                    ...
                                    input_ids_list,
                                    weights_list,
                                    image_paths=image_paths,  # 添加图像路径参数以支持3D融合
                                )
                            else:
                                input_ids = [ids.to(accelerator.device, non_blocking=True) for ids in
                    ...
                    optimizer.step()
                    if not (args.optimizer_type.lower().endswith(
                            "schedulefree") or args.optimizer_schedulefree_wrapper):
                        lr_scheduler.step()
