...
class LatentsCachingStrategy:
    ...
    def get_image_size_from_disk_cache_path(self, absolute_path: str, npz_path: str) -> Tuple[Optional[int], Optional[int]]:
        """
        Prefer parsing from filename suffix "_<W>x<H>_*.npz".
        Fallbacks:
        - Infer from NPZ contents if an array like (C,H,W) exists (assume stride=8 -> image H*8, W*8)
        - Use DEFAULT_IMAGE_W/H env vars or 512x512 as last resort
        """
        ...
        if len(parts) >= 2 and "x" in parts[-2]:
            try:
                ...
                pass
        # Fallback 1: infer from NPZ array shapes
        try:
            npz = np.load(npz_path)
            # try common keys first
            candidate_keys = list(npz.files)
            for key in candidate_keys:
                arr = npz[key]
                if isinstance(arr, np.ndarray) and arr.ndim == 3:
                    # assume CHW
                    _, h_lat, w_lat = arr.shape
                    # default SD VAE stride=8
                    return int(w_lat * 8), int(h_lat * 8)
        except Exception:
            pass
        # Fallback 2: env or defaults
        try:
            dw = int(os.environ.get("DEFAULT_IMAGE_W", "512"))
            dh = int(os.environ.get("DEFAULT_IMAGE_H", "512"))
            return dw, dh
        except Exception:
            return 512, 512
