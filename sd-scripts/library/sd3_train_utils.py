...
def save_sd3_model_on_train_end(
    ...
    def sd_saver(ckpt_file, epoch_no, global_step):
        ...
        # 训练结束时保存融合权重（若存在）
        try:
            enc_strategy = strategy_base.TextEncodingStrategy.get_strategy()
            if hasattr(enc_strategy, 'three_modal_fusion') and enc_strategy.three_modal_fusion is not None:
                fusion_container = enc_strategy.three_modal_fusion
                fusion_module = getattr(fusion_container, 'fusion', fusion_container)
                fusion_sd = fusion_module.state_dict()
                weights_path = ckpt_file.replace('.safetensors', '_fusion_weights.safetensors')
                save_file(fusion_sd, weights_path)
                logger.info(f"Saved fusion network weights -> {weights_path}")
        except Exception as e:
            logger.warning(f"Save fusion weights failed: {e}")
...
def do_sample(
    ...
    # Use configurable discrete flow shift to align with training (default 3.185 per train script)
    model_sampling = sd3_utils.ModelSamplingDiscreteFlow(shift=discrete_flow_shift)
        ...
        with torch.no_grad(), accelerator.autocast():
            ...
            for prompt_dict in prompts:
                ...
                if isinstance(resp, dict):
                    fusion_statuses.append(resp)
            if len(fusion_statuses) > 0:
                injected_cnt = sum(1 for x in fusion_statuses if x.get("injected"))
                missing_npz_cnt = sum(1 for x in fusion_statuses if x.get("injected") and x.get("npz") is None)
                logger.info(
                    f"FUSION_EPOCH_SUMMARY epoch={epoch} step={steps} total={len(fusion_statuses)} injected={injected_cnt} missing_npz={missing_npz_cnt}"
                )
    else:
...
def sample_image_inference(
    ...
    def encode_prompt(prpt, pointcloud_path=None):
        text_encoder_conds = []
        ...
        if text_encoders is not None:
            ...
            image_paths = None
            npz_used_path = None
            if pointcloud_path and hasattr(args, 'enable_3d_fusion') and args.enable_3d_fusion:
                # 从点云路径生成对应的NPZ特征文件路径（支持自定义目录，并使用项目根的绝对路径）
                import os, glob, re
                pc_filename = os.path.basename(pointcloud_path)
                sample_id = os.path.splitext(pc_filename)[0]  # 提取样本ID
                # 可选：强制按数字ID匹配（用于数据集按纯数字编号的情况）
                sample_id_lookup = sample_id
                if os.environ.get("FUSION_FORCE_NUMERIC_ID", "0") == "1":
                    m = re.search(r"(\d+)", sample_id)
                    if m:
                        numeric_id = m.group(1)
                        if numeric_id != sample_id:
                            # 若提供 POINTCLOUD_DIR，则尝试用数字ID改写点云路径以对齐数据集
                            pc_dir = os.environ.get("POINTCLOUD_DIR")
                            if pc_dir:
                                candidate_pt = os.path.join(pc_dir, f"{numeric_id}.pt")
                                candidate_npy = os.path.join(pc_dir, f"{numeric_id}.npy")
                                if os.path.exists(candidate_pt):
                                    pointcloud_path = candidate_pt
                                    pc_filename = os.path.basename(pointcloud_path)
                                    sample_id = os.path.splitext(pc_filename)[0]
                                elif os.path.exists(candidate_npy):
                                    pointcloud_path = candidate_npy
                                    pc_filename = os.path.basename(pointcloud_path)
                                    sample_id = os.path.splitext(pc_filename)[0]
                            sample_id_lookup = numeric_id
                # NPZ 目录：优先环境变量 NPZ_DIR，其次项目根的默认目录
                project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                npz_dir = os.environ.get("NPZ_DIR", os.path.join(project_root, "train", "data", "train", "aircraft"))
                npz_pattern = os.path.join(npz_dir, f"{sample_id_lookup}_*_sd3.npz")
                npz_files = glob.glob(npz_pattern)
                # 兼容同名无后缀：<id>.npz
                if not npz_files:
                    direct_npz = os.path.join(npz_dir, f"{sample_id_lookup}.npz")
                    if os.path.exists(direct_npz):
                        npz_files = [direct_npz]
                if npz_files:
                    image_path = npz_files[0]  # 使用第一个匹配的NPZ文件
                    # 将实际点云路径一并传给后端融合模块（通过自定义标记）
                    image_paths = [f"{image_path}|pc={os.path.abspath(pointcloud_path)}"]
                    npz_used_path = image_path
                    logger.info(f"Using pointcloud path for 3D fusion: {pointcloud_path} (sample_id: {sample_id_lookup}, npz: {os.path.basename(image_path)})")
                else:
                    # 尝试备用ID匹配：原始stem、去除“_features”后缀
                    alt_ids = [sample_id]
                    if sample_id.endswith("_features"):
                        alt_ids.append(sample_id[:-9])  # 去掉 _features
                    found = False
                    for alt_id in alt_ids:
                        alt_pattern = os.path.join(npz_dir, f"{alt_id}_*_sd3.npz")
                        alt_files = glob.glob(alt_pattern)
                        if not alt_files:
                            alt_direct = os.path.join(npz_dir, f"{alt_id}.npz")
                            if os.path.exists(alt_direct):
                                alt_files = [alt_direct]
                        if alt_files:
                            image_path = alt_files[0]
                            image_paths = [f"{image_path}|pc={os.path.abspath(pointcloud_path)}"]
                            npz_used_path = image_path
                            logger.info(f"Using alt sample_id '{alt_id}' for NPZ (npz: {os.path.basename(image_path)})")
                            found = True
                            break
                    if not found:
                        # 严格模式：要求存在NPZ，否则报错
                        if os.environ.get("FUSION_REQUIRE_NPZ", "0") == "1":
                            raise RuntimeError(
                                f"FUSION_REQUIRE_NPZ=1: NPZ not found for sample_id {sample_id_lookup} "
                                f"(tried pattern: {npz_pattern}, direct: {os.path.join(npz_dir, f'{sample_id_lookup}.npz')}, "
                                f"alts: {alt_ids})"
                            )
                        # 未找到NPZ时，也传入占位以启用三模态融合的零图像特征回退，同时保留真实点云文件路径
                        image_paths = [f"missing_npz://{sample_id_lookup}|pc={os.path.abspath(pointcloud_path)}"]
                        logger.warning(f"No NPZ file found for sample_id {sample_id_lookup}, using zero image feature fallback")
            
            # 仅在支持三模态融合且提供了image_paths时传入该参数；否则调用基础签名
            try:
                if hasattr(encoding_strategy, 'three_modal_fusion') and getattr(encoding_strategy, 'three_modal_fusion') is not None and image_paths is not None:
                    encoded_text_encoder_conds = encoding_strategy.encode_tokens(
                        tokenize_strategy, text_encoders, tokens_and_masks, image_paths=image_paths
                    )
                else:
                    encoded_text_encoder_conds = encoding_strategy.encode_tokens(
    ...
    # 从prompt_dict中获取点云路径；若缺失则从 prompt 文本解析 --pc <path>
    pointcloud_path = prompt_dict.get("pointcloud_path")
    if hasattr(args, 'enable_3d_fusion') and args.enable_3d_fusion:
        if not pointcloud_path and isinstance(prompt, str) and "--pc" in prompt:
            try:
                import re
                m = re.search(r"\s--pc\s+(\S+)", prompt)
                if m:
                    pointcloud_path = m.group(1)
                    # 从 prompt 中移除 --pc 片段，保持与训练编码一致
                    prompt = re.sub(r"\s--pc\s+\S+", "", prompt).strip()
            except Exception:
                pass
        if pointcloud_path:
            logger.info(f"Using pointcloud for 3D fusion: {pointcloud_path}")
        else:
            logger.debug("No pointcloud path specified; 3D fusion will fallback if NPZ missing")

    # 组装可观测的融合状态并输出标准日志
    fusion_enabled = hasattr(args, 'enable_3d_fusion') and args.enable_3d_fusion
    # 由于 encode_prompt 内部才解析/选择 npz，这里再次执行一次轻量解析以打印状态
    obs_pc = pointcloud_path if fusion_enabled and pointcloud_path else None
    obs_npz = None
    if fusion_enabled and obs_pc:
        try:
            import glob as _g
            import os as _os
            _pc_filename = _os.path.basename(obs_pc)
            _sample_id = _os.path.splitext(_pc_filename)[0]
            _project_root = _os.path.dirname(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))))
            _npz_dir = _os.environ.get("NPZ_DIR", _os.path.join(_project_root, "train", "data", "train", "aircraft"))
            # 与 encode_prompt 相同的查找顺序
            _lookup = _sample_id
            if os.environ.get("FUSION_FORCE_NUMERIC_ID", "0") == "1":
                import re as _re
                m = _re.search(r"(\d+)", _sample_id)
                if m:
                    _lookup = m.group(1)
            _pat = _os.path.join(_npz_dir, f"{_lookup}_*_sd3.npz")
            _files = _g.glob(_pat)
            if _files:
                obs_npz = _files[0]
            else:
                _alt_ids = [_sample_id]
                if _sample_id.endswith("_features"):
                    _alt_ids.append(_sample_id[:-9])
                for _aid in _alt_ids:
                    _alt_pat = _os.path.join(_npz_dir, f"{_aid}_*_sd3.npz")
                    _alt_files = _g.glob(_alt_pat)
                    if _alt_files:
                        obs_npz = _alt_files[0]
                        break
        except Exception:
            pass

    injected = int(fusion_enabled and obs_pc is not None)
    logger.info(
        f"FUSION_STATUS epoch={epoch} step={steps} enum={prompt_dict.get('enum')} injected={injected} "
        f"pc={'none' if obs_pc is None else os.path.abspath(obs_pc)} "
        f"npz={'missing' if (fusion_enabled and obs_pc and obs_npz is None) else (('none' if obs_npz is None else os.path.abspath(obs_npz)))}"
    )

    lg_out, t5_out, pooled, l_attn_mask, g_attn_mask, t5_attn_mask = encode_prompt(prompt, pointcloud_path)
    cond = encoding_strategy.concat_encodings(lg_out, t5_out, pooled)

    # encode negative prompts (negative prompts通常不需要3D特征)
    lg_out, t5_out, pooled, l_attn_mask, g_attn_mask, t5_attn_mask = encode_prompt(negative_prompt, None)
